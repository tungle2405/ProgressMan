...
namespace PMLecture.Context
{
    public class LopMonHocContext : ILopMonHoc
    {
        ...
        public List<SinhVienViewModel> GetAllSinhVienHocLopMonHoc(string maLopMonHoc)
        {
            var builder = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json", optional: false);
            IConfiguration config = builder.Build();
            string connectionString = config.GetValue<string>("ConnectionStrings:DefaultConnection");
            ...
            try
            {
                ...
                var sqlcon = DBConnection.GetSqlConnection(connectionString);
                SqlCommand cmd = new SqlCommand("GV_SP_GetAllSVHocLopMonHoc", sqlcon);
                ...
                while (reader.Read())
                {
                    ...
                    sinhVien.HoatDong = Convert.ToInt32(reader["HoatDong"].ToString().Trim());
        ...
        public CResponseMessage DeleteLopMonHoc(string maLopMH)
        {
            var builder = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json", optional: false);
            IConfiguration config = builder.Build();
            string connectionString = config.GetValue<string>("ConnectionStrings:DefaultConnection");
            ...
            try
            {
                ...
                var sqlcon = DBConnection.GetSqlConnection(connectionString);
                SqlCommand cmd = new SqlCommand("GV_SP_DeleteLopMonHoc", sqlcon);
                cmd.CommandType = CommandType.StoredProcedure;
        ...
        public CResponseMessage InsertSVHocLopMonHocTheoLop(string lopNienChe, string maLopMH)
        {
            var builder = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json", optional: false);
            IConfiguration config = builder.Build();
            string connectionString = config.GetValue<string>("ConnectionStrings:DefaultConnection");
        ...
        public CResponseMessage DeleteSVHocLopMonHocTheoLop(string MaSinhVien, string maLopMH)
        {
            var builder = new ConfigurationBuilder().SetBasePath(Directory.GetCurrentDirectory()).AddJsonFile("appsettings.json", optional: false);
            IConfiguration config = builder.Build();
            string connectionString = config.GetValue<string>("ConnectionStrings:DefaultConnection");
            ...
            try
            {
                ...
                var sqlcon = DBConnection.GetSqlConnection(connectionString);
                SqlCommand cmd = new SqlCommand("GV_SP_DeleteSinhVienCuaLopMH", sqlcon);
                cmd.CommandType = CommandType.StoredProcedure;
                cmd.Parameters.AddWithValue("@MaSinhVien", MaSinhVien);
                cmd.Parameters.AddWithValue("@MaLopMonHoc", maLopMH);
